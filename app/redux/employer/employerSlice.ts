...
export const addEmployerAsync = createAsyncThunk(
    ...
    async (employerData: Employer, { rejectWithValue }) => {
      try {
        ...
        return rejectWithValue(error.response?.data || "Failed to save employer details");
      }
    }
  );
  export const fetchEmployerAsync = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
      try {
        ...
        return rejectWithValue(error.response?.data || "Failed to fetch employer details");
      }
    }
  );
  export const updateEmployerAsync = createAsyncThunk(
    ...
    async (updatedEmployerData: EmployerResponse, { rejectWithValue }) => {
      try {
        ...
        console.error("API Error:", error.response?.data);
        return rejectWithValue(error.response?.data || "Failed to update employer details");
      }
    }
  );
  export const deleteEmployerAsync = createAsyncThunk(
    ...
    async (userId: number, { rejectWithValue }) => {
      try {
        ...
        console.error("API Error:", error.response?.data);
        return rejectWithValue(error.response?.data || "Failed to delete employer details");
      }
    }
  );
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return response.data.data; // ✅ array of employers
    } catch (error: any) {
      console.error("API Error:", error.response?.data);
  ...
  const employerSlice = createSlice({
    ...
    extraReducers: (builder) => {
      builder
        ...
        .addCase(addEmployerAsync.fulfilled, (state, action: PayloadAction<Employer>) => {
          state.loading = false;
        ...
        .addCase(fetchEmployerAsync.fulfilled, (state, action: PayloadAction<Employer>) => {
          state.loading = false;
          state.employer = action.payload;
        })
        ...
        .addCase(updateEmployerAsync.fulfilled, (state, action: PayloadAction<Employer>) => {
          state.loading = false;
          state.employer = action.payload; 
        })
    ...
    .addCase(fetchAllEmployersAsync.fulfilled, (state, action: PayloadAction<Employer[]>) => {
      state.loading = false;
